...
const AdminDashboard = () => {
  ...
  const [newDrill, setNewDrill] = useState({ title: '', youtube_url: '' });
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        {activeTab === 'roster' && data.roster.map((p: any) => (
          <div key={p.id} className="p-4 border-b flex justify-between font-bold">
            {p.full_name} 
            <span className="text-[#EF4444] text-xs font-black uppercase">{p.team_assigned || 'UNASSIGNED'}</span>
          </div>
        ))}
        ...
        {activeTab === 'schedule' && (
          <div>
            ...
            {data.games.map((g: any) => (
              <div key={g.id} className="p-4 border-b flex justify-between font-bold uppercase">{g.opponent} — {g.date}</div>
            ))}
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-8">
            <div className="flex justify-between mb-4">
              <h3 className="font-black uppercase">Schedule</h3>
              <X className="cursor-pointer" onClick={() => setIsGameModalOpen(false)} />
            </div>
