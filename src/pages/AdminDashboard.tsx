...
import { Shield, Users, Calendar, Video, X } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="p-20 text-center font-bold uppercase italic tracking-tighter">Syncing Bamika...</div>;
    ...
    <div className="max-w-7xl mx-auto p-4 space-y-6 font-sans">
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm min-h-[300px]">
        {activeTab === 'parents' && data.parents.map((u: any) => (
          <div key={u.id} className="p-4 border-b flex justify-between font-bold text-gray-700">{u.full_name} <Shield size={18} className="text-gray-200" /></div>
        ))}
        ...
        {activeTab === 'coaches' && data.coaches.map((c: any) => (
          <div key={c.id} className="p-4 border-b flex items-center gap-4 font-bold text-gray-700">
        ...
        {activeTab === 'schedule' && data.games.map((g: any) => (
          <div key={g.id} className="p-4 border-b font-bold uppercase text-gray-700 italic tracking-tight">{g.opponent} — {g.date}</div>
        ))}

        {activeTab === 'drills' && data.drills.map((d: any) => (
          <div key={d.id} className="p-4 border-b font-bold text-gray-700">{d.title}</div>
        ))}
      </div>
    </div>
  );
